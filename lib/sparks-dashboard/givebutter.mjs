...
export async function fetchGivebutterDashboard({ period = "fy", membershipType = "all" } = {}) {
  ...
  try {
    ...
    const prevNewInPeriod = members.filter((m) => isInRange(m.memberSince, prevRange)).length;
    const prevNewInPeriodPaid = paidMembers.filter((m) => isInRange(m.memberSince, prevRange)).length;
