...
function buildTags(row, context) {
  ...
  const onPageOne = row.position >= 4 && row.position <= 10;
  const onPageTwo = row.position >= 11 && row.position <= 20;

  if (onPageOne && row.impressions >= 30) tags.push("page_one");
  if (onPageTwo && row.impressions >= 30) tags.push("page_two");

  const strikingDistance =
    row.position >= 4 && row.position <= 20 && row.impressions >= 30;
  ...
  if (hasPrevious) {
    ...
    const risingClicks =
      clicksChange !== null && clicksChange >= 25 && row.impressions >= 20;
