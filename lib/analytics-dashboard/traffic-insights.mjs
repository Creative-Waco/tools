...
const MIN_PRIOR_SESSIONS_FOR_RISING = 10;
...
function deriveSourceAction({
  ...
  const isNewOrResurgent = (previousSessions ?? 0) < MIN_PRIOR_SESSIONS_FOR_RISING;
...
export function computeTrafficInsights({
  ...
  for (const row of landingPages) {
    ...
    if (row.bounceRate >= siteBounce + 8 || row.bounceRate >= 65) {
  ...
  for (const row of pages) {
    if (row.views < 40) continue;
    if (row.path.startsWith("/.wf_")) continue;

    const engagementGap = siteAvgEngagement - row.avgEngagementSec;
    if (row.avgEngagementSec >= siteAvgEngagement * 0.55) continue;
  ...
  for (const row of channels) {
    ...
    if (row.sessions >= 20 && change >= 20) {
      all.push({
        ...
        sessionsChange: change,
        impactScore:
          row.previousSessions >= MIN_PRIOR_SESSIONS_FOR_RISING
  ...
  if (programInsights) {
    ...
    for (const landing of programLandings.slice(0, 5)) {
      ...
      if (
        siteLanding &&
        siteLanding.bounceRate >= siteBounce + 12 &&
  ...
  const watchlist = uniqueAll
    .filter((row) =>
      row.tags.some((tag) =>
        ["declining_channel", "declining_source"].includes(tag),
      ),
    )
