...
import { buildVisitorJourney } from "./journey.mjs";
import { getProgram } from "./programs.mjs";

const INTERNAL_REFERRER_FILTER = {
  filter: {
    fieldName: "pageReferrer",
    stringFilter: {
      matchType: "CONTAINS",
      value: "creativewaco.org",
    },
  },
};
...
async function fetchSiteJourneyExtras(client, property, currentRange) {
  const [[landing], [nextPages], [events]] = await Promise.all([
    client.runReport({
      property,
      dateRanges: [currentRange],
    ...
    client.runReport({
      property,
      dateRanges: [currentRange],
      dimensionFilter: INTERNAL_REFERRER_FILTER,
  ...
  return {
    ...
    nextPages: (nextPages.rows ?? [])
      .filter((row) => {
        const path = row.dimensionValues?.[0]?.value ?? "";
        return path && !path.startsWith("/.wf_");
      })
      .map((row) => ({
        path: row.dimensionValues?.[0]?.value ?? "/",
...
export async function fetchAnalyticsDashboard({
  ...
  const [
    ...
    pageFilter
      ? Promise.resolve(null)
      : fetchSiteJourneyExtras(client, property, currentRange),
  ]);
  ...
  const visitorJourney = buildVisitorJourney({
    ...
    siteLandingPages: siteJourneyExtras?.landingPages,
    siteNextPages: siteJourneyExtras?.nextPages,
    siteEngagement: siteJourneyExtras?.engagement ?? null,
  });
