...
export function formatCardDateLabel(value) {
  const date = parseDate(value);
  if (!date) return "";
  ...
  const diffDays = zonedDayNumber(date) - zonedDayNumber(new Date());
...
export function shortLocationLabel(address) {
  const value = String(address ?? "").trim();
  if (!value) return "";

  const cityState = value.match(/([^,]+),\s*([A-Z]{2})\b/);
